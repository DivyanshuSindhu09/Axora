...
const Profile = () => {
  ...
  return user ? (
    <section className='relative min-h-screen max-h-screen font-[absans] overflow-y-scroll no-scrollbar p-4 sm:p-6'>
      <div className='max-w-3xl mx-auto'>

        {/* profile card */}
        <div className='bg-[#1A1D24] rounded-2xl shadow overflow-hidden'>
          {/* cover photo */}
          <div className='h-36 sm:h-56 bg-gradient-to-r from-indigo-200 via-purple-200 to-pink-200'>
            {user.cover_picture && (
              <img
        ...
        {/* tabs */}
        <div className='mt-4'>
          <div className='bg-[#1A1D24] overflow-hidden rounded-2xl shadow-2xl shadow-purple-500/20 p-1 flex max-w-md mx-auto border border-purple-500/20 backdrop-blur-sm'>
            {['posts', 'media', 'likes'].map((tab, index) => (
              <button
                onClick={()=>setActiveTab(tab)}
                className={`group relative flex-1 px-3 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-semibold rounded-xl transition-all duration-300 cursor-pointer overflow-hidden ${
                  activeTab === tab 
          ...
          {/* media tab */}
          {activeTab === "media" && (
            <div className='flex flex-wrap mt-4 sm:mt-6 gap-4 justify-center'>
              {posts.filter((post)=>post.image_urls.length > 0).map((post) => (
                post.image_urls.map((image, index)=>(
                  <Link
                    ...
                    key={index}
                  >
                    <img src={image} className='w-40 sm:w-64 aspect-video object-cover rounded-xl' />
                    <p className='absolute font-[absans] bottom-0 right-0 text-xs sm:text-sm p-1 px-2 sm:px-3 backdrop-blur-2xl text-white opacity-0 group-hover:opacity-100 transition duration-300'>
